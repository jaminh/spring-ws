...
/**
 * A WS-Security endpoint interceptor based on Apache's WSS4J. This interceptor
 * supports messages created by the
 ...
 *
 * <p>
 * The validation and securement actions executed by this interceptor are
 * configured via {@code validationActions} and {@code securementActions}
 * properties, respectively. Actions should be passed as a space-separated
 * strings.
 *
 * <p>
...
public class Wss4jSecurityInterceptor extends AbstractWsSecurityInterceptor implements InitializingBean {
	 ...
	 *
	 * <p>
	 * The value of the actor or role has to match the receiver's setting or may
	 * contain standard values.
	 */
	 ...
	 * recommends to use the identifier type {@code IssuerSerial}. For possible
	 * encryption key identifier types refer to
	 * {@link org.apache.ws.security.handler.WSHandlerConstants#keyIdentifier}.
	 * For encryption {@code IssuerSerial}, {@code X509KeyIdentifier},
	 * {@code DirectReference}, {@code Thumbprint}, {@code SKIKeyIdentifier},
	 * and {@code EmbeddedKeyName} are valid only.
	 */
	 ...
	 * Currently WSS4J supports {@link WSConstants#KEYTRANSPORT_RSA15} and
	 ...
	 *
	 * <p>
	 * The value of this property is a list of semicolon separated element names
	 * that identify the elements to encrypt. An encryption mode specifier and a
	 * namespace identification, each inside a pair of curly brackets, may
	 * precede each element name.
	 *
	 * <p>
	 * The encryption mode specifier is either {@code {Content}} or
	 * {@code {Element}}. Please refer to the W3C XML Encryption specification
	 * about the differences between Element and Content encryption. The
	 * encryption mode defaults to {@code Content} if it is omitted. Example of
	 * a list:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The first entry of the list identifies the element {@code CreditCard} in
	 * the namespace {@code http://example.org/paymentv2}, and will encrypt its
	 * content. Be aware that the element name, the namespace identifier, and
	 * the encryption modifier are case sensitive.
	 *
	 * <p>
	 * The encryption modifier and the namespace identifier can be omitted. In
	 * this case the encryption mode defaults to {@code Content} and the
	 * namespace is set to the SOAP namespace.
	 *
	 * <p>
	 * An empty encryption mode defaults to {@code Content}, an empty namespace
	 * identifier defaults to the SOAP namespace. The second line of the example
	 * defines {@code Element} as encryption mode for an {@code UserName}
	 * element in the SOAP namespace.
	 *
	 * <p>
	 * To specify an element without a namespace use the string {@code Null} as
	 * the namespace name (this is a case sensitive string)
	 *
	 * <p>
	 * If no list is specified, the handler encrypts the SOAP Body in
	 * {@code Content} mode by default.
	 */
	 ...
	 * {@link WSConstants#AES_128}, {@link WSConstants#AES_256}, and
	 * {@link WSConstants#AES_192}. Except for AES 192 all of these algorithms
	 * are required by the XML Encryption specification.
	 */
	 ...
	 *
	 * <p>
	 * If this parameter is not set, then the encryption function falls back to
	 * the {@link org.apache.ws.security.handler.WSHandlerConstants#USER}
	 * parameter to get the certificate.
	 *
	 * <p>
	 * If <b>only</b> encryption of the SOAP body data is requested, it is
	 * recommended to use this parameter to define the username. The application
	 * can then use the standard user and password functions (see example at
	 * {@link org.apache.ws.security.handler.WSHandlerConstants#USER} to enable
	 * HTTP authentication functions.
	 *
	 * <p>
	 * Encryption only does not authenticate a user / sender, therefore it does
	 * not need a password.
	 *
	 * <p>
	 * Placing the username of the encryption certificate in the configuration
	 * file is not a security risk, because the public key of that certificate
	 * is used only.
	 */
	...
	/**
	 * Specific parameter for UsernameToken action to define the encoding of the
	 * passowrd.
	 *
	 * <p>
	 ...
	 * Defines which key identifier type to use. The WS-Security specifications
	 * recommends to use the identifier type {@code IssuerSerial}. For possible
	 * signature key identifier types refer to
	 * {@link org.apache.ws.security.handler.WSHandlerConstants#keyIdentifier}.
	 * For signature {@code IssuerSerial} and {@code DirectReference} are valid
	 * only.
	 */
	 ...
	 *
	 * <p>
	 * Refer to {@link #setSecurementEncryptionParts(String)} for a detailed
	 * description of the format of the value string.
	 *
	 * <p>
	 * If this property is not specified the handler signs the SOAP Body by
	 * default.
	 *
	 * <p>
	 * The WS Security specifications define several formats to transfer the
	 * signature tokens (certificates) or references to these tokens. Thus, the
	 * plain element name {@code Token} signs the token and takes care of the
	 * different formats.
	 *
	 * <p>
	 ...
	 * {@code Body} then the SOAP namespace identifier can be empty
	 * ({@code {}}).
	 */
	 ...
	 *
	 * <p>
	 * If this parameter is not set, then the signature function falls back to
	 * the alias specified by {@link #setSecurementUsername(String)}.
	 *
	 */
	...
	/**
	 * Sets the username for securement username token or/and the alias of the
	 * private key for securement signature
	 */
	...
	/**
	 * Sets the {@link org.apache.ws.security.WSPasswordCallback} handler to use
	...
	/**
	 * Sets the {@link org.apache.ws.security.WSPasswordCallback} handlers to
	 * use when validating messages.
	 *
	...
	/**
	 * Whether to enable signatureConfirmation or not. By default
	 * signatureConfirmation is enabled
	 */
	...
	/**
	 * Sets whether or not timestamp verification is done with the server-side
	 * time to live
	 */
	...
	/**
	 * Enables the {@code mustUnderstand} attribute on WS-Security headers on
	 * outgoing messages. Default is {@code true}.
	 */
	 ...
	 * 
	 * @param config
	 *            web service security configuration or {@code null} to use
	 *            default settings
	 */
	...
	/**
	 * Set the WS-I Basic Security Profile compliance mode. Default is
	 * {@code true}.
	 */
	...
	/**
	 * Sets the time in seconds in the future within which the Created time of
	 * an incoming Timestamp is valid. The default is 60 seconds.
	 */
	...
	/**
	 * Checks whether the received headers match the configured validation
	 * actions. Subclasses could override this method for custom verification
	 * behavior.
	 *
	 *
	 * @param results
	 *            the results of the validation function
	 * @param validationActions
	 *            the decoded validation actions
	 * @throws Wss4jSecurityValidationException
	 *             if the results are deemed invalid
	 */
	...
	/**
	 * Puts the results of WS-Security headers processing in the message
	 * context. Some actions like Signature Confirmation require this.
	 */
